...
from reposec.detectors.deps import (
    fixed_version_for,
    parse_manifests,
    scan_dependencies,
    severity_of,
)
from reposec.detectors.osv import OSVUnavailable, Package
...
def _stub_osv(monkeypatch, hits, details):
    monkeypatch.setattr(deps, "query_batch", lambda pkgs: hits(pkgs))
    monkeypatch.setattr(deps, "fetch_details", lambda ids: details)
...
def test_scan_reports_degraded_when_osv_is_unreachable(monkeypatch):
    ...
    monkeypatch.setattr(deps, "query_batch", boom)
    findings, degraded = scan_dependencies([("requirements.txt", "requests==2.19.0")])
